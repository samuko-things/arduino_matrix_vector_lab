...
public:
    ...
    void directionCosinesRad(dtype (&result)[3], dtype1 (&vector1)[3])
    {
        double buffer[3];
    ...
    // this function calculates the Direction Cosine in radians
    template <typename dtype, typename dtype1>
    void directionCosinesDeg(dtype (&result)[3], dtype1 (&vector1)[3])
    {
        double buffer[3];
        double unit_vector[3];
        normalize(unit_vector, vector1);
    ...
    dtype angleBtwRad(dtype1 (&vector1)[3], dtype2 (&vector2)[3])
    {
        ...
        double angle = acos((unit_vector1[0] * unit_vector2[0]) + (unit_vector1[1] * unit_vector2[1]) + (unit_vector1[2] * unit_vector2[2]));
        return (dtype)angle;
    }
    ...
    dtype angleBtwDeg(dtype1 (&vector1)[3], dtype2 (&vector2)[3])
    {
        ...
        double angle = acos((unit_vector1[0] * unit_vector2[0]) + (unit_vector1[1] * unit_vector2[1]) + (unit_vector1[2] * unit_vector2[2])) * 180 / M_PI;
        return (dtype)angle;
    }
...
public:
    ...
    template <typename dtype, typename dtype1, typename dtype2, SIZE N>
    void dot(dtype (&result)[N], dtype1 (&matrix)[N][N], dtype2 (&vector1)[N])
    {
        double buffer[N];
        double val;
        ...
        for (SIZE row = 0; row < N; row += 1)
        {
            for (SIZE col = 0; col < 1; col += 1)
            {
                for (SIZE count = 0; count < N; count += 1)
                {
                    val += (double)matrix[row][count] * (double)vector1[count];
                }
                buffer[row] = val;
                val = 0;
            }
